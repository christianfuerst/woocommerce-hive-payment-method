...
class WC_Hive {
	...
	public static function wc_set_cart_cookies($set) {
		...
		if (self::get('amount_currency') == null) {
			// Initialize WC_Hive variables for the new cart.
			WC_Hive::reset();
		}
	}	
	...
	public static function generate_memo($length = 10) {
		...
		return apply_filters('wc_hive_generate_memo', $memo);
	}
	...
	public static function get($key) {
		...
		return WC()->session->get("wc_hive_{$key}");
	}
	...
	public static function set($key, $value) {
		...
		return WC()->session->set("wc_hive_{$key}", $value);
	}
