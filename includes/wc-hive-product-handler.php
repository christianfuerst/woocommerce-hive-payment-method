...
class WC_Hive_Product_Handler {
	...
	public static function init() {
		...
		if (wc_hive_get_setting('show_insightful') == 'yes') {
	...
	/**
	 * Display prices in single product in different accepted Hive currencies
	 *
	 * @since 1.0.4
	 */
	...
	/**
	 * Display prices in different accepted Hive currencies
	 *
	 * @since 1.0.4
	 */
	public static function display_prices($product = null) {
		$from_currency_symbol = wc_hive_get_currency_symbol(); ?>

		<?php if ($currencies = wc_hive_get_accepted_currencies()) : ?>
			<div class="wc-hive wc-hive-prices">
					...
					<?php if ( ! empty($prices['price'])) : ?>
						...
						<?php foreach ($currencies as $to_currency_symbol) : ?>
							<?php if ($min_price !== $max_price) : ?>
								<div class="wc-hive-price">
									<?php self::display_price_range(
										wc_hive_rate_convert($min_price, $from_currency_symbol, $to_currency_symbol),
										wc_hive_rate_convert($max_price, $from_currency_symbol, $to_currency_symbol),
										$to_currency_symbol
									); ?>
								</div>
							<?php elseif ($product->is_on_sale() && $min_reg_price === $max_reg_price) : ?>
								...
								<?php self::display_price_sale(
										wc_hive_rate_convert($min_reg_price, $from_currency_symbol, $to_currency_symbol),
										wc_hive_rate_convert($max_reg_price, $from_currency_symbol, $to_currency_symbol),
										$to_currency_symbol
									); ?>
								</div>
							<?php else : ?>
								<div class="wc-hive-price">
									<?php self::display_price(wc_hive_rate_convert($min_price, $from_currency_symbol, $to_currency_symbol), $to_currency_symbol); ?>
								</div>
							<?php endif; ?>
						<?php endforeach; ?>
					<?php endif; ?>
					...
					<?php if ($product->get_price() > 0) : ?>
						<?php if ($product->is_on_sale()) : ?>
							<?php foreach ($currencies as $to_currency_symbol) : ?>
								<div class="wc-hive-price">
									<?php self::display_price_sale(
										wc_hive_rate_convert($product->get_regular_price(), $from_currency_symbol, $to_currency_symbol), 
										wc_hive_rate_convert($product->get_price(), $from_currency_symbol, $to_currency_symbol),
										$to_currency_symbol); 
									?>
								</div>
							<?php endforeach; ?>
						<?php else : ?>
							<?php foreach ($currencies as $to_currency_symbol) : ?>
								<div class="wc-hive-price">
									<?php self::display_price(wc_hive_rate_convert($product->get_price(), $from_currency_symbol, $to_currency_symbol), $to_currency_symbol); ?>
								</div>
							<?php endforeach; ?>
						<?php endif; ?>
					<?php endif; ?>
	...
	/**
	 * Attach the Hive currency prices to a variation
	 *
	 * @since 1.0.4
	 * @param array $data
	...
	public static function display_price($price, $currency_symbol) { ?>
		<span class="wc-hive-price-amount <?php echo sanitize_title(sprintf('wc-hive-price-%s', $currency_symbol)); ?>">
			<strong><?php echo $price; ?></strong>
			<?php if ( $currency_symbol ) { ?>
			<span class="wc-hive-price-amount-symbol"><?php echo $currency_symbol; ?></span>
			<?php } ?>
		</span>
		<?php
	}
	...
	public static function display_price_sale($regular_price, $sale_price, $currency_symbol) { ?>
		...
		<?php if (wc_hive_get_setting('show_discounted_price') == 'yes') { ?>
		<del>
